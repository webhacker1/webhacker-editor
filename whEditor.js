...
export default class WebHackerEditor {
  ...
  renderEditorInterface() {
    ...
    historyGroupElement.append(
      this.createToolbarButton("fa-solid fa-rotate-left", "Отменить", () => executeRichCommand("undo")),
      this.createToolbarButton("fa-solid fa-rotate-right", "Повторить", () => executeRichCommand("redo"))
    );

    const textGroupElement = createElement("div", "webhacker-toolbar__group");
    textGroupElement.append(
      this.createToolbarButton("fa-solid fa-bold", "Жирный", () => executeRichCommand("bold"), true, "bold"),
      this.createToolbarButton("fa-solid fa-italic", "Курсив", () => executeRichCommand("italic"), true, "italic"),
      this.createToolbarButton("fa-solid fa-underline", "Подчеркнуть", () => executeRichCommand("underline"), true, "underline"),
      this.createToolbarButton("fa-solid fa-strikethrough", "Зачеркнуть", () => executeRichCommand("strikeThrough"), true, "strikeThrough"),
      this.createColorDropdown(),
      this.createLinkDropdown(),
      this.createImageDropdown(),
      this.createToolbarButton("fa-solid fa-eraser", "Очистить форматирование", () => executeRichCommand("removeFormat"))
    );
    ...
    alignGroupElement.append(
      this.createToolbarButton("fa-solid fa-align-left", "По левому краю", () => executeRichCommand("justifyLeft"), true, "alignLeft"),
      this.createToolbarButton("fa-solid fa-align-center", "По центру", () => executeRichCommand("justifyCenter"), true, "alignCenter"),
      this.createToolbarButton("fa-solid fa-align-right", "По правому краю", () => executeRichCommand("justifyRight"), true, "alignRight")
    );

    const listsGroupElement = createElement("div", "webhacker-toolbar__group");
    listsGroupElement.append(
      this.createToolbarButton("fa-solid fa-list-ul", "Маркированный список", () => executeRichCommand("insertUnorderedList"), true, "unorderedList"),
      this.createToolbarButton("fa-solid fa-list-ol", "Нумерованный список", () => executeRichCommand("insertOrderedList"), true, "orderedList"),
      this.createToolbarButton("fa-solid fa-table", "Таблица 2×2", () => this.insertMinimalTable())
    );

    toolbarElement.append(
      historyGroupElement, this.createSeparator(),
      textGroupElement, this.createSeparator(),
      structureGroupElement, this.createSeparator(),
      alignGroupElement, this.createSeparator(),
      listsGroupElement
    );

    const contentEditableElement = createElement("div", "webhacker-content", { contenteditable: "true" });
    contentEditableElement.setAttribute("data-placeholder", this.editorOptions.placeholderText);
  ...
  createColorDropdown() {
    ...
    DEFAULT_TEXT_PRESET_COLORS.forEach(hexColor => {
      const swatchButtonElement = createElement("button", "webhacker-swatch", { type: "button", "data-color": hexColor, title: hexColor });
      swatchButtonElement.style.background = hexColor;
      swatchButtonElement.addEventListener("click", () => {
        swatchesContainerElement.querySelectorAll(".webhacker-swatch").forEach(node => node.removeAttribute("aria-selected"));
      ...
      swatchButtonElement.addEventListener("dblclick", () => applySelectedColorAndClose(hexColor));
      swatchesContainerElement.appendChild(swatchButtonElement);
    });
    const colorPreviewElement = createElement("div", "webhacker-color__preview");
    ...
    const colorHexInputElement = createElement("input", "webhacker-input", { type: "text", maxlength: "7", value: "#6a5acd", placeholder: "#RRGGBB", "aria-label": "HEX цвет" });
    ...
    const colorActionsRowElement = createElement("div", "webhacker-actions");
    const confirmButtonElement = createElement("button", "webhacker-button webhacker-button--primary", { type: "button" });
    confirmButtonElement.textContent = "OK";
    const resetButtonElement = createElement("button", "webhacker-button webhacker-button--ghost", { type: "button" });
    resetButtonElement.textContent = "Сброс";
    colorActionsRowElement.append(confirmButtonElement, resetButtonElement);
    colorContainerElement.append(swatchesContainerElement, colorPreviewElement, colorRowElement, colorActionsRowElement);
    ...
    colorPickerInputElement.addEventListener("input", handlePickerInputChange);
    ...
    confirmButtonElement.addEventListener("click", () => {
      ...
      if (!/^#[0-9a-fA-F]{6}$/.test(hexColor)) { colorHexInputElement.focus(); return; }
  ...
  createLinkDropdown() {
    ...
    const linkTextInputElement = createElement("input", "webhacker-input", { type: "text", placeholder: "Текст ссылки (необязательно)" });
    const linkActionsRowElement = createElement("div", "webhacker-actions");
    const linkConfirmButtonElement = createElement("button", "webhacker-button webhacker-button--primary", { type: "button" });
    linkConfirmButtonElement.textContent = "OK";
    const linkRemoveButtonElement = createElement("button", "webhacker-button webhacker-button--ghost", { type: "button" });
    ...
    linkConfirmButtonElement.addEventListener("click", () => {
      ...
      const linkHtml = `<a href="${hrefValue}" target="_blank" rel="noopener noreferrer">${escapeHtml(labelValue || visibleText)}</a>`;
  ...
  createImageDropdown() {
    ...
    const imageFileInputElement = createElement("input", "webhacker-file", { type: "file", accept: "image/*" });
    const imageActionsRowElement = createElement("div", "webhacker-actions");
    const imageConfirmButtonElement = createElement("button", "webhacker-button webhacker-button--primary", { type: "button" });
    imageConfirmButtonElement.textContent = "OK";
    const imageRemoveButtonElement = createElement("button", "webhacker-button webhacker-button--ghost", { type: "button" });
    imageRemoveButtonElement.textContent = "Удалить";
    imageActionsRowElement.append(imageConfirmButtonElement, imageRemoveButtonElement);
    imageFormElement.append(imageUrlInputElement, imageFileInputElement, imageActionsRowElement);
    dropdownMenuElement.appendChild(imageFormElement);
    const insertImageAtSelection = srcValue => {
      this.closeAllMenus();
      this.contentEditableElement.focus();
      this.restoreSelectionRange(this.currentSavedSelectionRange);
      executeRichCommand("insertHTML", `<img src="${escapeHtml(srcValue)}" alt="">`);
      this.emitChange();
      this.syncToggleStates();
    };
    imageConfirmButtonElement.addEventListener("click", () => {
      const urlValue = imageUrlInputElement.value.trim();
      if (urlValue) { insertImageAtSelection(ensureSafeUrl(urlValue)); return; }
      const fileObject = imageFileInputElement.files && imageFileInputElement.files[0];
  ...
  bindEditorEvents() {
    ...
    document.addEventListener("selectionchange", () => {
      const selection = window.getSelection();
      if (!selection || selection.rangeCount === 0) return;
      const anchorNode = selection.anchorNode && selection.anchorNode.nodeType === 3 ? selection.anchorNode.parentNode : selection.anchorNode;
      if (anchorNode && this.contentEditableElement.contains(anchorNode)) this.syncToggleStates();
    });
    this.contentEditableElement.addEventListener("keydown", event => {
      const pressedKey = event.key.toLowerCase();
      const hasCommandModifier = event.ctrlKey || event.metaKey;
      if (hasCommandModifier && pressedKey === "b") { event.preventDefault(); executeRichCommand("bold"); this.emitChange(); this.syncToggleStates(); }
      if (hasCommandModifier && pressedKey === "i") { event.preventDefault(); executeRichCommand("italic"); this.emitChange(); this.syncToggleStates(); }
      if (hasCommandModifier && pressedKey === "u") { event.preventDefault(); executeRichCommand("underline"); this.emitChange(); this.syncToggleStates(); }
  ...
  insertMinimalTable(rowCount = 2, columnCount = 2) {
    ...
    for (let rowIndex = 0; rowIndex < rowCount; rowIndex += 1) {
      tableBodyHtml += "<tr>";
      for (let columnIndex = 0; columnIndex < columnCount; columnIndex += 1) tableBodyHtml += "<td>Ячейка</td>";
      tableBodyHtml += "</tr>";
    }
    tableBodyHtml += "</tbody>";
